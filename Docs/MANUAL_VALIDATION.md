...
- [ ] With MacWatch running, execute `./Scripts/generate-test-telemetry.sh`. It temporarily creates a disabled, uniquely named test plist in `~/Library/LaunchAgents`, modifies it, and removes it. It also opens and closes loopback-only TCP and UDP listeners. Confirm `startupAdded`, `startupModified`, `startupRemoved`, `listeningEndpointOpened`, and `listeningEndpointClosed`. Its exit trap removes the test artifacts if interrupted.
- [ ] Separately run `swift run MacWatchSelfTest` to verify scanner logic against a unique temporary directory. This validates code but does not generate events in the running MacWatch app.
